  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity style={s.row}
        onPress={() => {
          markRead(item.id);
          navigation.navigate('Chat', { chatId: item.id, chatName: getName(item) });
        }}>
...
const s = StyleSheet.create({
  ...
  fab: {
    ...
    backgroundColor: 'rgba(0,200,83,0.15)', alignItems: 'center',
    justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(0,200,83,0.3)',
  },
  row: {
  ...
  unreadDot: {
    ...
    backgroundColor: '#00c853', marginLeft: 8,
  },
